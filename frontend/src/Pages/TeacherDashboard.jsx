...
export const TeacherDashboard = ({ Name }) => {
  ...
  async function createClass(data) {
    try {
      ...
      const response = await fetch("http://localhost:5000/api/teacher/new-class", {
        method: "POST",
        headers: {
  ...
  async function deleteClass(classId) {
    try {
      const user = await authWithFirebase.getAuthUser();
      const token = await user.getIdToken();
      const response = await fetch("http://localhost:5000/api/teacher/deletion-class", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  async function connectAdmin(data) {
    try {
      const user = await authWithFirebase.getAuthUser();
      const token = await user.getIdToken();
      const response = await fetch("http://localhost:5000/api/teacher/admin-connection", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    async function classFetching() {
      try {
        const user = await authWithFirebase.getAuthUser();
        const classesObject = await authWithFirebase.fetchingClassData(user.uid);
